...
class libjpegConan(ConanFile):
    ...
    def build(self):
        ...
        shared_definition = "-DSHARED_LIB=1" if self.options.shared else ""
        if self.settings.os == "Windows":
            self.run("IF not exist build mkdir build")
        else:
            self.run("mkdir build")
        cd_build = "cd build"
        self.output.warn('%s && cmake .. %s %s' % (cd_build, cmake.command_line, shared_definition))
        self.run('%s && cmake .. %s %s' % (cd_build, cmake.command_line, shared_definition))
        ...
        if self.settings.os == "Windows":
            if self.options.shared:
                ...
                if self.settings.build_type == "Debug":
                    self.copy(pattern="*.pdb", dst="bin", src="build", keep_path=False)

                self.copy(pattern="libjpegd.lib", dst="lib", src="build", keep_path=False)
                self.copy(pattern="libjpeg.lib", dst="lib", src="build", keep_path=False)
            else:
                self.copy(pattern="libjpegd.lib", dst="lib", src="build", keep_path=False)
                self.copy(pattern="libjpeg.lib", dst="lib", src="build", keep_path=False)
        else:
            if self.options.shared:
                if self.settings.os == "Macos":
                    self.copy(pattern="*.dylib", dst="lib", src="build/libjpeg", keep_path=False)
                else:
                    self.copy(pattern="*.so*", dst="lib", src="build/libjpeg", keep_path=False)
            else:
